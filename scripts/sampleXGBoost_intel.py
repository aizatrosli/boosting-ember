...
with mlflow.start_run(run_name="demo_ember_xgboost_cpuboost") as run:
    ...
    starttime = time.time()
    model = xgb.XGBClassifier(booster="dart", objective="binary:logistic", n_jobs=20)
    model.fit(X_train, y_train)
    model.get_booster().feature_names = emberfeatures().features
    mlflow.log_metric('fit_time', time.time()-starttime)
    mlflow.log_param('n_split', nsplit)
    mlflow.sklearn.eval_and_log_metrics(model=model, X=X_test, y_true=y_test, prefix='main_')
    ...
    if cvrun:
        for ix, (train_ix, test_ix) in enumerate(TimeSeriesSplit(n_splits=nsplit).split(X_train)):
            with mlflow.start_run(run_name=f'cross_validation_{ix}', nested=True) as child_run:
                ...
                mlflow.sklearn.eval_and_log_metrics(model=cvmodel, X=X_test, y_true=y_test, prefix=f'{ix}_')
